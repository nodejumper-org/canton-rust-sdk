//! A CIP-0112 transfer, end to end — the V2 counterpart of `v1_transfer`.
//!
//! Same shape as V1: ask the registry who administers the instrument, build the
//! transfer from *generated* types, let the registry resolve the factory and
//! supply the choice context, and submit with the contracts it said to
//! disclose. Read the two side by side and the differences are the whole story:
//!
//! * the transfer moves between **accounts**, not parties — set
//!   `CANTON_TOKEN_SENDER_ACCOUNT` / `CANTON_TOKEN_RECEIVER_ACCOUNT` to name
//!   which one, since a party can hold several;
//! * the choice names its **actors** — the sender, here;
//! * the paths carry `/v2/`, which the client handles.
//!
//! ```sh
...
/// `provider` is left unset — a registry that routes through one names it, and
/// one that does not would reject a value it did not expect.
fn account(owner: rt::Party, id: String) -> h::Account {
    h::Account {
        owner: Some(owner),
        provider: None,
        id,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // As in V1: read from the registry here for brevity, which only checks that
    // it did not change between this call and the factory call. Naming the
    // administrator protects against a registry substituting one only when the
    // value comes from somewhere the caller trusts.
    ...
    let sender_account = account(
        sender.clone(),
        std::env::var("CANTON_TOKEN_SENDER_ACCOUNT").unwrap_or_default(),
    );
    let receiver_account = account(
        receiver,
        std::env::var("CANTON_TOKEN_RECEIVER_ACCOUNT").unwrap_or_default(),
    );
    println!(
    ...
    let transfer = ti::Transfer {
        // The V1 difference in one line: accounts, not parties.
    ...
    // Resolve the factory and take its context — and say who is exercising,
    // which V1 left implicit in the submitting party.
    let command = canton_token::v2::transfer(&registry, transfer, vec![sender.clone()]).await?;
