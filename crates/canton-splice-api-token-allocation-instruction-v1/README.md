...
Pre-built typed Rust bindings for the Token Standard allocation-instruction API (`splice-api-token-allocation-instruction-v1`), generated from
`splice-api-token-allocation-instruction-v1-1.0.0.dar` by [`canton-codegen`](https://crates.io/crates/canton-codegen).

```sh
cargo add canton-splice-api-token-allocation-instruction-v1
```
